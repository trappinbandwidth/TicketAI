"""
Carver — primary extraction agent.
...
def _run_extraction(state: TicketState, pass_num: int, temperature: float) -> tuple[dict, bool, dict | None]:
    ...
    logger.warning("[carver_pass1] pass=%d START file=%s temp=%.2f", pass_num, filename, temperature)
    ...
    try:
        ...
        logger.warning("[carver_pass1] pass=%d OK file=%s fields=%d/%d empty=%s",
        ...
        logger.error("[carver_pass1] pass=%d FAILED file=%s error=%s", pass_num, filename, exc, exc_info=True)
...
def carver_pass1(state: TicketState) -> dict:
    filename = state.get("filename", "unknown")
    logger.warning("[carver_pass1] START file=%s pages=%d ocr_chars=%d",
                   filename,
...
def carver_pass2(state: TicketState) -> dict:
